...
- **Palette Favorites Export/Import & Backups**: `promptc palette favorites --export PATH` writes a portable JSON snapshot, `--import-from PATH` (+ `--replace`) restores/merges favorites, and every mutation now creates a timestamped `.bak` of `~/.promptc_ui.json` (last 5 retained). New quality-of-life flags: `--list-stale` / `--prune-stale` to surface and clean invalid command ids, and `--reorder id1,id2,...` to set explicit ordering.
