    ...
    suite = TestSuite(
        ...
        test_cases=[
            ...
            TestCase(
                ...
                assertions=[Assertion(type="not_contains", value="fails")],
            ),
        ],
    )
...
def test_runner_can_assert_policy_risk_and_execution_mode(tmp_path):
    p_file = tmp_path / "policy_prompt.txt"
    p_file.write_text(
        "Analyze my stock portfolio and suggest an investment strategy.", encoding="utf-8"
    )
    ...
    suite = TestSuite(
        ...
        test_cases=[
            TestCase(
                id="policy-1",
                assertions=[
                    Assertion(type="risk_at_least", target="policy", value="high"),
                    Assertion(
                        type="execution_mode_is", target="policy", value="human_approval_required"
                    ),
                ],
            )
        ],
...
def test_runner_can_assert_policy_contains_and_ir_includes(tmp_path):
    p_file = tmp_path / "debug_prompt.txt"
    p_file.write_text(
    ...
    suite = TestSuite(
        ...
        test_cases=[
            TestCase(
                id="intent-1",
                assertions=[
                    Assertion(type="policy_contains", target="policy", value="workspace_read"),
                    Assertion(type="includes", target="ir", value="developer"),
                ],
            )
        ],
    )
