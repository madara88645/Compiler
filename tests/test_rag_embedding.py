...
def test_cli_embedding_roundtrip(tmp_path: Path):
    ...
    r = runner.invoke(cli_app, ['rag', 'index', str(tmp_path), '--db-path', str(db_path), '--embed'])
    assert r.exit_code == 0, r.output
    # embedding query
    qr = runner.invoke(cli_app, ['rag', 'query', 'alpha beta', '--db-path', str(db_path), '--method', 'embed', '--json'])
    ...
    lr = runner.invoke(cli_app, ['rag', 'query', 'alpha beta', '--db-path', str(db_path), '--method', 'fts', '--json'])
...
def test_api_embedding(tmp_path: Path):
    ...
    # ingest with embeddings
    res = client.post('/rag/ingest', json={'paths':[str(tmp_path)], 'db_path': db_path, 'embed': True})
    assert res.status_code == 200, res.text
    j = res.json()
    assert j['ingested_docs'] >= 2
    # embed query
    q = client.post('/rag/query', json={'query': 'red green', 'db_path': db_path, 'method': 'embed'})
