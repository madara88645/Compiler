...
def test_rag_index_and_query(tmp_path: Path):
    ...
    # Index the folder
    result = runner.invoke(app, ["rag", "index", str(src), "--db-path", str(db_path), "--ext", ".txt", "--ext", ".md"])
    assert result.exit_code == 0, result.output
    assert db_path.exists(), "Database file should be created"

    # Query a term from the .txt file
    q1 = runner.invoke(app, ["rag", "query", "gradient descent", "--db-path", str(db_path), "--k", "3", "--json"])
    ...
    q2 = runner.invoke(app, ["rag", "query", "teaching persona", "--db-path", str(db_path), "--k", "3", "--json"])
