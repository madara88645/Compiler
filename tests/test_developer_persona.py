...
def test_live_debug_en_constraints():
    ir = compile_text("Live debug this traceback and help me reproduce with a minimal repro in Python")
    assert ir.persona in {"developer","assistant"}
    joined = " | ".join(ir.constraints).lower()
...
def test_live_debug_tr_constraints():
    ir = compile_text("Canlı debug: bu hata ayıklamada yığın izini analiz edip minimal örnek oluştur")
    assert ir.persona in {"developer","assistant"}
    j = " | ".join(ir.constraints).lower()
