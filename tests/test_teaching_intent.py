...
def test_teaching_intent_tr():
    ...
    assert (
        any("öğretici" in c or "pedagog" in c for c in [x.lower() for x in ir.constraints])
        or ir.steps
    )
...
def test_teaching_intent_en():
    ...
    assert (
        any("pedagogical" in c or "progressive" in c for c in [x.lower() for x in ir.constraints])
        or ir.steps
    )
    assert "friendly" in [x.lower() for x in ir.tone]
