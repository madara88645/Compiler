...
def test_rag_pack_cli_args(monkeypatch):
    ...
    def mock_pack(query, results, max_chars=4000, max_tokens=None, token_chars=4.0, dedup=False, token_aware=False):
        captured_args['dedup'] = dedup
        captured_args['token_aware'] = token_aware
        return {"packed": "mock", "included": [], "chars": 0, "tokens": 0, "query": query, "budget": {}}

    # Patch the function as imported in cli.main
    monkeypatch.setattr(cli.main, "pack_context", mock_pack)
    
    # Run command
    result = runner.invoke(app, ["rag", "pack", "test search", "--dedup", "--token-aware", "--max-tokens", "100"])
