...
* **CLI: Palette Favorites (new)**: Manage command palette favorites from the CLI with `promptc palette commands`/`promptc palette favorites`; shared storage with desktop UI via `~/.promptc_ui.json` (override with `PROMPTC_UI_CONFIG`), export/import collections via `--export` / `--import-from`, automatically keep timestamped backups of the config file on every change, list/prune stale commands with `--list-stale` / `--prune-stale`, and reorder favorites with `--reorder id1,id2,...`
