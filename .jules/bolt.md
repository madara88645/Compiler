...
## 2024-05-18 - Fast String Joining with Multipliers
**Learning:** In CPython, when building identical placeholder strings for SQL queries (e.g., `?,?,?`), using list multiplication with `.join()` (`",".join(["?"] * len(batch_ids))`) is significantly faster (~5x) than using a generator expression (`",".join("?" for _ in batch_ids)`). It avoids the overhead of setting up a generator, iterating through it in Python bytecode, and allows the C backend of `.join()` to optimally pre-allocate memory because the size of the list is known immediately.
**Action:** When generating repeated identical strings or placeholders, prefer `str.join(["string"] * count)` over generator loops.
