...
## 2026-06-25 - Replace any() generator expression with fast-path `in` operator
**Learning:** In string list existence checks, using an inline `any(x == val for x in list)` generator expression creates overhead. The cost of setting up and tearing down the generator frame eclipses the cost of the actual `in` operation, especially since the `in` operator leverages native C implementation for list scanning.
**Action:** Replace `any()` generator expressions used for string existence checks in lists with the explicit `in` operator to bypass generator overhead and achieve a speedup.
