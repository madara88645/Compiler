...
## 2026-07-23 - Avoid optimizing bounded loops with negligible gains
**Learning:** In `app/readiness/reference_rules.py`, an `any()` generator expression operates on a list (`found`) that is bounded to a maximum of 5 elements due to the function's own `return found[:5]` constraint. Replacing this generator expression with a multi-line explicit `for` loop trades readability for an unmeasurable gain because the collection size is too small to make generator overhead a bottleneck. The `# Bolt Optimization:` comment also added noise for the next reader without explaining a constraint the code couldn't already show.
**Action:** Do not sacrifice readability by removing idiomatic generator expressions like `any()` when the underlying collection is statically or dynamically bounded to a very small size (e.g., 5 elements). Only optimize loops that run across significant data volumes or are proven to be measurable hot paths.
