...
## 2024-07-25 - Removing any() generator overhead in heuristic string existence checks
**Learning:** In heavily utilized heuristic paths, such as `app/heuristics/__init__.py`, using an inline `any(ev == "software:frontend_download_feature" for ev in evidence)` generator expression creates a measurable performance bottleneck. The overhead of setting up and tearing down the generator frame eclipses the cost of the actual string `in` operation. Microbenchmarks show a ~4-5x performance improvement by replacing it with a simple `"software:frontend_download_feature" in evidence` check.
**Action:** Replace `any()` generator expressions used for exact string existence checks in lists with the `in` operator to bypass generator overhead and achieve a 4-5x speedup.
