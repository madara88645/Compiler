...
## 2025-02-27 - Regex Findall vs Finditer Performance
**Learning:** When counting regex matches and extracting the first match, `len(pattern.findall(text))` combined with `pattern.search(text)` is significantly faster (around 2x in benchmarks) than using `next(pattern.finditer(text))` and `sum(1 for _ in matches)`. This is because `findall` executes entirely at the C level, avoiding the overhead of generating match objects and iterating over them in Python bytecode.
**Action:** Use `findall()` when you only need the match data (strings) or count, and avoid `finditer()` unless you specifically need the match objects (e.g., for start/end indices or groups of *every* match).
