...
## 2025-06-25 - Prevent Unauthorized LLM Usage via Fallback Dependencies
**Vulnerability:** Cost-incurring endpoints (e.g., `/compile`, `/rag/query`, `/agent-generator/export`) were using the optional `verify_api_key_if_required` dependency instead of strict `verify_api_key`.
**Learning:** Using optional authentication dependencies (like checking `PROMPTC_REQUIRE_API_KEY_FOR_ALL`) on critical routes exposes them to unauthenticated resource exhaustion and API abuse by default.
**Prevention:** Always enforce strict authentication (`Depends(verify_api_key)`) on all endpoints that trigger external API calls or high-cost operations, regardless of global configuration toggles.
