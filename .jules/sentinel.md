...
## 2026-05-15 - SQLite Bulk Query Parameter DoS
**Vulnerability:** A Denial of Service (DoS) vulnerability existed in the RAG retrieval flow (`app/rag/simple_index.py`). When fetching full metadata for matched embeddings via `SELECT ... WHERE c.id IN ({placeholders})`, the `chunk_ids` array was not batched. Because SQLite enforces a strict limit on query parameters (often 999 or 32766), searches that matched thousands of document chunks would raise an `OperationalError` and crash the server.
**Learning:** Even internal query generation logic must account for database engine constraints. Dynamically sizing `IN` clauses based on unfiltered query results creates a hard crash threshold when the underlying data grows beyond the database engine's host parameter limit.
**Prevention:** Always batch bulk lookup arrays (e.g., iterating in slices of 999 elements) before constructing `IN` clauses for SQLite, ensuring the query size remains within safe, predictable limits.
