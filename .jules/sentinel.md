...
## 2025-06-25 - Prevent In-Memory Dictionary Memory Leak in Rate Limiter
...
## 2025-05-27 - Prevent Information Leakage in API Middleware
**Vulnerability:** Information Exposure (CWE-200) in FastAPI error handling middleware. Unhandled exceptions raised an `HTTPException` where the raw `Exception` was re-raised or leaked out if not caught carefully within custom middleware overrides. The `log_requests` middleware intercepted the error, logged it, but then used `raise` which could leak the internal Python stack trace directly to the client depending on the downstream ASGI server configuration or exception handlers.
**Learning:** In a custom Starlette/FastAPI HTTP middleware designed to log errors and wrap responses, simply re-raising the caught exception with `raise` after logging leaves the final response generation up to the global exception handler, which might inadvertently leak `detail` strings or stack traces. Overriding the return directly with a safe `JSONResponse` guarantees what the client receives.
**Prevention:** When capturing exceptions within `app.middleware("http")`, explicitly catch `Exception as exc`, skip over `HTTPException`s that are designed to be public (by re-raising them), but for all other unhandled internal server errors, return a strict generic `JSONResponse(status_code=500, content={"detail": "An internal error occurred."})` to safely terminate the request and prevent information leakage.
