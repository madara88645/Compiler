...
## 2025-07-01 - Prevent Internal Exception Details Leak in Generator API
**Vulnerability:** The `/repo-context/github`, `/skills-generator/generate`, and `/agent-generator/generate` endpoints in `api/routes/generators.py` were returning raw exception details `str(exc)` directly to the client via HTTP 400 and 500 errors.
**Learning:** Returning raw exception details is a major security risk (Information Exposure) as it can leak sensitive internal implementation details, such as file paths, database schemas, third-party API keys, or stack traces.
**Prevention:** Catch exceptions, log their details internally (e.g., via `logger.exception`), and raise an `HTTPException` with a generic `detail` message (like "An internal error occurred." or "Invalid GitHub repository URL.") rather than returning `str(exc)` directly to the user.
