...
## 2025-07-01 - Prevent Application Crash on Null/Undefined in escapeHtml

**Vulnerability:** The custom `escapeHtml` function in `extension/popup.js` expected a string and called `.replaceAll` directly on it. If a non-string value (like `null` or `undefined`) was passed due to upstream data inconsistencies, the application would throw a TypeError and crash the popup UI, creating a fragility point. While not a direct XSS, this fragility could be exploited to cause a Denial of Service within the extension.
**Learning:** Functions designed to sanitize or escape data before rendering must be aggressively defensive against unexpected input types. Relying on implicit typing in JavaScript for critical rendering paths often leads to brittle code that fails abruptly when data contracts are slightly violated.
**Prevention:** Always defensively cast inputs to strings (e.g., using `String(value ?? "")`) before performing string-specific operations like `.replaceAll` in custom sanitization or escaping utilities.
