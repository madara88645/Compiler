...
## 2025-06-25 - Prevent Information Exposure in API Export Router
**Vulnerability:** The `/export` endpoint in `api/routes/export.py` reflected raw user input (`req.format`) directly back in the `HTTPException` detail message (e.g., `detail=f"Unsupported format: {req.format}"`). This constitutes an Information Exposure vulnerability (CWE-200) and potential log pollution or client-side reflection risk.
**Learning:** Returning unsanitized user input in HTTP error responses is an anti-pattern. While FastAPI generally returns JSON, reflecting arbitrary input can still cause issues if clients misinterpret the response or if it triggers unexpected downstream logging behavior.
**Prevention:** Never reflect raw, unvalidated user input directly in HTTP error `detail` messages. Always use static, safe, generic error strings (e.g., `detail="Unsupported format."`) instead of formatting user-provided variables into the message.
