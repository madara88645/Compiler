...
## 2025-06-25 - Fix Timing Attack in Admin Key Verification
**Vulnerability:** The `_matches_admin_api_key` function in `api/auth.py` contained a timing attack vulnerability. While it attempted to mitigate leaking the key length by performing a dummy `secrets.compare_digest` if the lengths differed, the first length check `if len(normalized_key) == len(admin_key):` still resulted in a slight timing difference, as the successful path only called `compare_digest` once, whereas the failed path executed a branch and a dummy `compare_digest`.
**Learning:** Even with dummy operations, branching based on string length before a constant-time comparison can introduce subtle timing variations.
**Prevention:** To prevent length-based timing attacks, ensure the `compare_digest` operation is always executed exactly once, using a padded or dummy string of the exact same length as the expected secret if the input length is incorrect, and returning the logical AND of the length check and the digest check.
