...
## 2025-06-25 - Fix Information Leakage via HTTP Exceptions
**Vulnerability:** The `api/routes/generators.py` endpoint returned raw exception strings (e.g., `str(exc)`) directly in the HTTP 400 and 404/502 responses for custom exceptions like `InvalidGitHubRepoUrl` and `GitHubRepoAnalysisError`.
**Learning:** Even when using custom, application-specific exception classes, returning `str(exc)` in API responses is risky because the underlying exception messages might dynamically include sensitive upstream errors, file paths, or internal formats.
**Prevention:** When raising HTTPException in FastAPI routes, do not pass `str(exc)` (even for custom application exceptions) to the `detail` parameter, as it can inadvertently expose sensitive upstream errors or internal paths. Always use safe, hardcoded generic error messages for the client.
