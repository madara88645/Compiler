...
## 2026-05-07 - Prevent fallback to deprecated ADMIN_API_KEY for proxy authentication
**Vulnerability:** The web backend proxy would fallback to the deprecated `ADMIN_API_KEY` if `PROMPTC_SERVER_API_KEY` was not configured, potentially exposing the admin key cross-service.
**Learning:** Relying on fallback environment variables for different security scopes (like admin vs. proxy) violates the Principle of Least Privilege. It allows components to unintentionally assume higher privileges.
**Prevention:** Strict separation of API keys. Never allow components to fallback to highly privileged keys. Enforce correct configuration per service scope and document expected variables.
