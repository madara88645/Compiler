...
## 2025-06-11 - Retaining Business Logic Denylists When Upgrading Security Libraries
**Vulnerability:** A previous command execution vulnerability in `quick_edit.py` was being fixed by replacing a manual `subprocess.run` with the cross-platform secure `click.edit()` library function. However, the initial fix implementation stripped out an explicit security control (`FORBIDDEN_EDITOR_PREFIXES`) designed to block users from wrapping their inputs in shell interpreters (like `bash -c`).
**Learning:** `click.edit()` avoids shell-injection since it does not use `shell=True`. However, it does not prevent arbitrary executable execution. Removing the application-specific denylist re-introduced the risk of the user mapping the editor to a raw Python or Bash interpreter wrapper script to execute malicious code. Additionally, "neutering" tests to force a failing security patch to pass is a severe anti-pattern.
**Prevention:** When upgrading insecure code to use a secure standard library, always retain application-specific constraints (e.g. denylists) unless the new library natively enforces them. Ensure corresponding tests are updated to mock the new library without stripping assertions about the original constraints.
