...
function getConnectionBadge(isConnected: boolean | null): {
    label: string;
    tone: string;
} {
    if (isConnected === true)
        return { label: "Connected", tone: "text-green-400 bg-green-500/10" };
    ...
    return (
        <div className="flex flex-col gap-4 border-t border-white/5 pt-4 mt-4">
            <h3 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest flex items-center justify-between gap-2">
                ...
                <span
                    className={`text-[9px] px-1.5 py-0.5 rounded ${connectionBadge.tone}`}
                >
            ...
            {indexStats && indexStats.docs > 0 && (
                <div className="flex gap-4 px-3 py-2 bg-white/5 rounded-lg border border-white/5">
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="text-xs font-mono text-zinc-300">
                            {(indexStats.total_bytes / 1024).toFixed(1)} KB
                        </span>
                    </div>
                </div>
            )}
            ...
            {status && (
                <div
                    aria-live="polite"
                    className={`text-[10px] px-2 py-1.5 rounded-lg ${getStatusTone(status)}`}
                >
                    {status}
                </div>
            )}
            ...
            <details className="group">
                ...
                <div className="flex flex-col gap-2 mt-2 pl-2 border-l border-white/5">
                    ...
                    <button
                        ...
                        title={
                            !filePath
                                ? "Enter a file path first to ingest"
                                : "Ingest Path"
                        }
