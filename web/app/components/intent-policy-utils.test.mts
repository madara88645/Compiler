...
test("normalizeIntentPolicy returns ordered intent details with human-friendly labels", async () => {
    const { humanizeIntentPolicyValue, normalizeIntentPolicy } =
        await import("./intent-policy-utils.ts");
    ...
    assert.equal(
        humanizeIntentPolicyValue(result.allowedTools[0]),
        "Workspace Read",
    );
    assert.equal(
        humanizeIntentPolicyValue(result.allowedTools[1]),
        "Run Tests",
    );
    assert.equal(
        humanizeIntentPolicyValue(result.forbiddenTools[0]),
        "Write Outside Workspace",
    );
    assert.equal(
        humanizeIntentPolicyValue(result.sanitizationRules[0]),
        "Mask Sensitive Values",
    );
    assert.equal(
        humanizeIntentPolicyValue(result.executionMode),
