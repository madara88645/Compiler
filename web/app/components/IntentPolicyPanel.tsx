...
import {
    humanizeIntentPolicyValue,
    normalizeIntentPolicy,
    type IntentDisplayGroup,
} from "./intent-policy-utils";
...
function FieldList({
    ...
    return (
        <div className="rounded-2xl border border-white/5 bg-black/20 p-4">
            <div className="mb-3 text-[11px] font-mono uppercase tracking-[0.18em] text-zinc-500">
                {title}
            </div>
            {items.length > 0 ? (
...
function IntentGroupBadge({ group }: { group: IntentDisplayGroup }) {
    ...
    return (
        <span
            className={`rounded-full border px-2.5 py-1 text-[10px] font-mono uppercase tracking-[0.18em] ${styles[group]}`}
        >
            {group}
        </span>
    );
    ...
    return (
        <div className="h-full overflow-y-auto p-6 text-zinc-200">
            <div className="grid gap-4 lg:grid-cols-2">
                        ...
                        <div>
                            ...
                            {intentPolicy.intentDetails.length > 0 ? (
                                <div className="grid gap-3">
                                    {intentPolicy.intentDetails.map(
                                        (intent) => (
                                            <div
                                                key={intent.key}
                                ...
                                <div className="rounded-2xl border border-dashed border-white/10 bg-black/20 p-4">
                                    ...
                                    <div className="mt-1 text-sm text-zinc-500">
                                        No special intent signals were inferred,
                                        so the compiler is treating this as a
                        ...
                        <div>
                            ...
                            <div className="mt-1 text-sm text-zinc-200">
                                {intentPolicy.riskDomains.length > 0
                                    ? intentPolicy.riskDomains
                                          .map(humanizeIntentPolicyValue)
                                          .join(", ")
                                    : "None"}
                            </div>
                        </div>
                ...
                <FieldList
                    title="Allowed Tools"
                    items={intentPolicy.allowedTools.map(
                        humanizeIntentPolicyValue,
                    )}
                ...
                <FieldList
                    ...
                    items={intentPolicy.forbiddenTools.map(
                        humanizeIntentPolicyValue,
                    )}
                ...
                <div className="lg:col-span-2">
                    <FieldList
                        ...
                        items={intentPolicy.sanitizationRules.map(
                            humanizeIntentPolicyValue,
                        )}
