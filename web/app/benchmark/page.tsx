...
export default function BenchmarkPage() {
    ...
    const [benchmarkResult, setBenchmarkResult] =
        useState<BenchmarkPayload | null>(null);
    ...
    const selectedModelMeta = useMemo(
        () =>
            selectedModel === "mock"
                ? null
                : getBenchmarkModelById(selectedModel),
        [selectedModel],
    );
    ...
    const generateMockResult = useCallback((): BenchmarkPayload => {
        ...
        return {
            ...
            metrics: {
                safety: {
                    raw: +(6 + Math.random() * 2).toFixed(1),
                    compiled: +(9 + Math.random()).toFixed(1),
                },
                clarity: {
                    raw: +(5 + Math.random() * 2).toFixed(1),
                    compiled: +(8 + Math.random() * 2).toFixed(1),
                },
                conciseness: {
                ...
                <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
                    <div className="z-20 flex w-full md:w-[350px] md:shrink-0 flex-col gap-5 border-b md:border-b-0 md:border-r border-white/5 bg-black/20 p-5">
                        ...
                        <button
                            ...
                            title={
                                !prompt.trim()
                                    ? "Enter a prompt first to start battle"
                                    : "Start Battle"
                            }
                    ...
                    <div className="relative flex flex-1 flex-col overflow-hidden bg-black/10">
                        {benchmarkResult ? (
                            <div className="flex h-full flex-1 flex-col overflow-hidden animate-fade-in">
                                ...
                                <div className="flex min-h-0 flex-1 flex-col bg-black/20">
                                    <div className="flex-1 overflow-hidden p-4">
                                        <DiffViewer
                                            oldText={benchmarkResult.raw_output}
                                            newText={
                                                benchmarkResult.compiled_output
                                            }
                                        />
                                    </div>
                                </div>
                            ...
                            <div className="flex flex-1 flex-col items-center justify-center gap-6 p-10 text-center animate-fade-in">
                                <div
                                    className={`text-6xl drop-shadow-2xl ${errorMessage ? "text-red-300" : "text-zinc-400"}`}
                                >
                                    {errorMessage ? "!" : "B"}
                                </div>
                                <div className="max-w-sm space-y-2">
                                    ...
                                    {!errorMessage && (
                                        <button
                                            ...
                                            className="mt-6 px-4 py-2 bg-amber-600/20 text-amber-300 border border-amber-500/30 rounded-xl hover:bg-amber-600/30 text-sm font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 flex items-center gap-2 mx-auto"
                                        >
                                            Set up Battle
                                        </button>
                                    )}
