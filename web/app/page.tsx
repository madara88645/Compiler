        ...
        >
            <div className="max-w-md rounded-lg border border-red-500/20 bg-red-500/10 p-6 shadow-xl shadow-red-950/10">
                ...
                <p className="mt-3 text-xs leading-relaxed text-zinc-500">
                    Your prompt is still in the editor. Retry after checking the
                    backend or API URL.
                </p>
                <button
                    type="button"
...
export default function Home() {
    ...
    const [conservativeMode, setConservativeMode] = useState(() => {
        ...
        return (
            window.localStorage.getItem("promptc_conservative_mode") !== "false"
        );
    });
    ...
    useEffect(() => {
        window.localStorage.setItem(
            "promptc_conservative_mode",
            String(conservativeMode),
        );
    }, [conservativeMode]);

    const activeMode: CompileMode = conservativeMode
        ? "conservative"
        : "default";
            ...
            <div className="glass w-full max-w-7xl h-full max-h-[90vh] rounded-3xl flex flex-col shadow-2xl overflow-hidden animate-fade-in ring-1 ring-white/10">
                ...
                <header className="border-b border-white/5 bg-black/20 p-4 flex items-center justify-between backdrop-blur-md">
                    <div className="flex items-center gap-3">
                        <div className="flex items-center gap-3">
                            <div className="h-9 w-9 bg-gradient-to-br from-blue-600 to-indigo-600 rounded-xl flex items-center justify-center text-lg text-white shadow-lg shadow-blue-500/20">
                                💠
                            </div>
                            <div>
                        ...
                        >
                            <span
                                className={`h-2.5 w-2.5 rounded-full transition-all ${
                                    conservativeMode
                                        ? "bg-emerald-400 shadow-[0_0_10px_rgba(74,222,128,0.7)]"
                                        : "bg-zinc-500"
                                }`}
                            />
                    ...
                    <div className="w-full md:w-[35%] min-h-0 p-5 flex flex-col gap-5 border-r border-white/5 bg-black/10">
                        ...
                        <div className="flex flex-col gap-4">
                            <button
                                ...
                                title={
                                    !prompt.trim()
                                        ? "Enter a prompt first to compile"
                                        : "Compile Prompt"
                                }
                        ...
                        <ContextManager
                            ...
                            suggestions={
                                result?.ir?.metadata?.context_suggestions
                            }
                        />
                    </div>
                        ...
                        {!!lastError && !loading ? (
                            ...
                            <>
                                ...
                                <div
                                    ...
                                    className="flex border-b border-white/5 px-4 pt-4 gap-2 overflow-x-auto scroll-smooth"
                                    style={{
                                        maskImage:
                                ...
                                >
                                    {(
                                        ...
                                        <button
                                            ...
                                            className={`px-4 py-2 text-[13px] font-medium rounded-t-lg transition-all relative whitespace-nowrap focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:z-10 ${
                                                activeTab === tab
                                                ...
                                                <div className="absolute top-4 right-6 z-10 flex gap-2">
                                                    ...
                                                    {result.ir?.metadata
                                                        ...
                                                        result.ir.metadata
                                                            ...
                                                            .length > 0 && (
                                                            <div
                                                                className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-indigo-500/10 border border-indigo-500/20 backdrop-blur-md"
                                                    ...
                                                    {result.critique && (
                                                        <div
                                                            tabIndex={0}
                                                            className={`flex items-center gap-1.5 px-2 py-1 rounded-md border backdrop-blur-md cursor-help group/critic relative focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${
                                                                result.critique
                                                                    .verdict ===
                                                                "REJECT"
                                                        ...
                                                        >
                                                            <div
                                                                className={`w-1.5 h-1.5 rounded-full ${result.critique.verdict === "REJECT" ? "bg-red-400" : "bg-green-400"}`}
                                                            />
                                                            <span
                                                                className={`text-[10px] font-medium ${result.critique.verdict === "REJECT" ? "text-red-200" : "text-green-200"}`}
                                                            >
                                                                Critic:{" "}
                                                                {
                                                                    result
                                                            ...
                                                            <div className="absolute top-8 right-0 w-64 bg-zinc-900 border border-white/10 rounded-xl p-3 shadow-2xl opacity-0 invisible group-hover/critic:opacity-100 group-hover/critic:visible group-focus-visible/critic:opacity-100 group-focus-visible/critic:visible transition-all z-50">
                                                                <div className="flex justify-between items-center mb-2">
                                                                    ...
                                                                    <span
                                                                        className={`text-[10px] px-1.5 py-0.5 rounded ${result.critique.verdict === "REJECT" ? "bg-red-900 text-red-300" : "bg-green-900 text-green-300"}`}
                                                                    >
                                                                        {
                                                                            result
                                                                                .critique
                                                                        ...
                                                                        {result.critique.issues.map(
                                                                            (
                                                                                ...
                                                                                <div
                                                                                    ...
                                                                                    className="text-[9px] bg-black/20 p-1.5 rounded border border-white/5 text-zinc-400"
                                                                                >
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center gap-6 p-10 text-center">
                                ...
                                <div className="max-w-sm space-y-2">
                                    <h3 className="text-zinc-100 font-semibold tracking-tight text-base">
                                        Start with any rough idea
                                    </h3>
                                    <p className="text-sm text-zinc-400 leading-relaxed">
                                        Paste a task, question, or workflow on
                                        the left, then press{" "}
                                        <kbd className="text-[11px] font-mono border border-white/20 rounded px-1 py-0.5 bg-white/5">
                                            Ctrl/⌘ Enter
                                        </kbd>
                                        . You&apos;ll get a clean system &amp;
                                        user prompt, an execution plan, and
                                        safety checks — ready to drop into any
                                        LLM.
                                    </p>
                                    ...
                                    <button
                                        ...
                                        className="mt-6 px-4 py-2 bg-blue-600/20 text-blue-300 border border-blue-500/30 rounded-xl hover:bg-blue-600/30 text-sm font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 flex items-center gap-2 mx-auto"
                                    >
                                        Start Compiling
                                    </button>
                                </div>
