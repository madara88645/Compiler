...
function normalizeOptimizeResponse(
    data: Partial<OptimizeResponse>,
): OptimizeResponse {
    const beforeTokens = toFiniteNumber(data.before_tokens);
    const afterTokens = toFiniteNumber(data.after_tokens);
    const calculatedSavedPercent =
        beforeTokens > 0
            ? Number(
                  (((beforeTokens - afterTokens) / beforeTokens) * 100).toFixed(
                      1,
                  ),
              )
            : 0;
    ...
    return {
        ...
        saved_percent: toFiniteNumber(
            data.saved_percent,
            calculatedSavedPercent,
        ),
        ...
        tokenizer_method: toStringValue(
            data.tokenizer_method,
            DEFAULT_TOKENIZER_METHOD,
        ),
        estimated_input_cost_usd: toFiniteNumber(data.estimated_input_cost_usd),
        estimated_output_cost_usd: toFiniteNumber(
            data.estimated_output_cost_usd,
        ),
                ...
                <header className="border-b border-white/5 bg-black/20 p-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between backdrop-blur-md">
                    ...
                    <div className="flex flex-col gap-3 rounded-lg border border-white/10 bg-zinc-950/40 p-3 sm:flex-row sm:items-center">
                        ...
                        <button
                            ...
                            title={
                                !input.trim()
                                    ? "Enter a prompt first to analyze cost"
                                    : "Analyze cost"
                            }
                    ...
                    <div className="grid min-h-[50vh] grid-cols-1 gap-5 lg:grid-cols-2">
                        <section className="flex min-h-80 flex-col gap-2">
                            <label
                                ...
                                className="ml-1 text-xs font-bold uppercase tracking-wider text-zinc-500"
                            >
                                Original Prompt
                            </label>
                        ...
                        <section className="flex min-h-80 flex-col gap-2">
                            ...
                            <div className="min-h-0 flex-1 rounded-lg border border-emerald-500/20 bg-emerald-950/10 p-4">
                                {output ? (
                                    ...
                                    <div className="flex flex-col h-full min-h-72 items-center justify-center gap-4 text-sm italic text-zinc-400">
                                        ...
                                        <button
                                            ...
                                            className="px-4 py-2 bg-emerald-600/20 text-emerald-300 border border-emerald-500/30 rounded-xl hover:bg-emerald-600/30 text-sm font-medium not-italic transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 flex items-center gap-2"
                                        >
                                            Analyze Cost
                                        </button>
                                    </div>
                                    ...
                                    <div className="mt-3 flex flex-col gap-3">
                                        ...
                                        <div className="flex items-center justify-between gap-3">
                                            <span className="text-xs text-zinc-500">
                                                Estimated cost:{" "}
                                                {formatUsd(
                                                    result.english_variant_cost_usd,
                                                )}
                                            </span>
                                            <button
                                                type="button"
                                                onClick={() =>
                                                    copyText(englishVariant)
                                                }
