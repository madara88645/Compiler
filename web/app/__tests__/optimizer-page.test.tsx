...
describe("Optimizer page", () => {
    ...
    it("shows Groq cost estimates and a separate English suggestion", async () => {
        ...
        fireEvent.change(screen.getByLabelText("Original Prompt"), {
            target: {
                value: "Bu PDF'i ozetle ve junior gelistirici icin uygulama plani yaz.",
            },
        });
        fireEvent.click(screen.getByTitle(/Analyze cost/i));
        ...
        expect(
            screen.getByText(
                "Translation can change nuance; review before using.",
            ),
        ).toBeTruthy();

        fireEvent.click(
            screen.getByRole("button", { name: "Copy English variant" }),
        );
