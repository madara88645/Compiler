...
class OptimizationScreen(Screen):
    ...
    def on_mount(self) -> None:
        self.query_one("#activity-log").write(
            "[dim]Ready to optimize. Press 's' to start simulation.[/dim]"
        )
...
class TUIEvolutionCallback:
    ...
    def on_complete(self, best_candidate: Candidate) -> None:
        self.app.call_from_thread(
            self._update_log, f"[bold gold]DONE! Best Score: {best_candidate.score}[/bold gold]"
        )
        self.app.call_from_thread(
            lambda: setattr(self.screen.query_one("#btn-start", Button), "disabled", False)
        )
    ...
    def _handle_new_best(self, candidate: Candidate, score: float) -> None:
        ...
        # Update Log
        self.screen.query_one("#activity-log").write(
            f"[cyan]New Best:[/cyan] {score:.2f} ({candidate.mutation_type})"
        )
