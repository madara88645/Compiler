...
class DomainHandler(BaseHandler):
    ...
    def detect_implied_persona(self, text: str) -> Tuple[Optional[str], float]:
        ...
        for pattern_obj, persona_name in self._compiled_implied_personas.items():
            # Count occurrences
            # Bolt Optimization: using len(findall) and search() is significantly faster than
            # finditer with next() and sum() because it evaluates regex matches entirely in C.
