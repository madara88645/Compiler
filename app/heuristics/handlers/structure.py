...
class StructureHandler:
    ...
    def _segment_sections(self, text: str) -> Dict[str, str]:
        ...
        for part in parts:
            ...
            if any(x in lower_part for x in ["act as", "you are", "your role", "role:"]):
                current_bucket = "Role"
            elif any(x in lower_part for x in ["context:", "background:", "situation:", "context is"]):
                current_bucket = "Context"
            elif any(x in lower_part for x in ["task:", "goal:", "objective:", "do this:", "please", "write a", "create a", "your task", "task is"]):
                current_bucket = "Task"
            elif any(x in lower_part for x in ["constraint", "avoid", "do not", "limit:", "rule", "don't"]):
  ...
  <schema>
    ...
    def _format_deepspec(self, sections: Dict[str, str], variables: List[str], output_xml: str) -> str:
        """
