...
class PromptComparator:
    ...
    def compare(
        self, prompt_a: str, prompt_b: str, label_a: str = "Prompt A", label_b: str = "Prompt B"
    ) -> ComparisonResult:
        """
    ...
    def _compare_ir(
        self, ir_a: Dict[str, Any], ir_b: Dict[str, Any]
    ) -> Tuple[str, List[Dict[str, Any]]]:
        """IR farklarını bul"""
        # JSON'a çevir (pretty print)
        json_a = orjson.dumps(ir_a, option=orjson.OPT_INDENT_2 | orjson.OPT_SORT_KEYS).decode("utf-8")
        json_b = orjson.dumps(ir_b, option=orjson.OPT_INDENT_2 | orjson.OPT_SORT_KEYS).decode("utf-8")
    ...
    def _generate_recommendation(
        ...
        recommendation = (
            f"**{better_label}** daha iyi (skor farkı: {abs(score_diff):.1f} puan).\n\n"
        )
    ...
    def _compare_categories(
        self, validation_a: ValidationResult, validation_b: ValidationResult
...
def compare_prompts(
    prompt_a: str, prompt_b: str, label_a: str = "Prompt A", label_b: str = "Prompt B"
) -> ComparisonResult:
    """
